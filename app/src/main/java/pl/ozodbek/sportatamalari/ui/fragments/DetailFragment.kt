...
class DetailFragment : Fragment() {
    ...
    private fun scrollButtonSetUp() {
        binding.verticalButton.setOnClickListener {
            if (binding.pdfViewer.isSwipeVertical) {
                val currentOffset = binding.pdfViewer.currentYOffset
                val animator = ValueAnimator.ofFloat(currentOffset, 0f).apply {
                    duration = 700
                    addUpdateListener {
                        binding.pdfViewer.moveTo(1f, it.animatedValue as Float)
                    }
                }
        ...
        binding.horizontalButton.setOnClickListener {
            if (!binding.pdfViewer.isSwipeVertical) {
                val currentOffset = binding.pdfViewer.currentXOffset
                val animator = ValueAnimator.ofFloat(currentOffset, 0f).apply {
                    duration = 700
                    addUpdateListener {
                        binding.pdfViewer.moveTo(it.animatedValue as Float, 0f)
                    }
                }
                binding.pdfViewer.fitToWidth()
                animator.start()
    ...
    private fun horizontalPageSetUp() {
        ...
        pdfFileNames[args.chapterPostion]?.let {
            ...
            val savedPageNumber = sharedPref.getInt(SAVED_PAGE, 0)
